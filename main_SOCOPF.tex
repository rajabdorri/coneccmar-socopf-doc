...
\begin{enumerate}
  \item resuelve a optimalidad en una sola pasada y es numéricamente
  ajustada (brecha máxima $\sim 10^{-7}$);
  ...
  tensiones dentro de $\SI{0.003}{pu}$;
  \item bajo el escenario de costes considerado, satura el enlace
  \gls{HVDC} España--Italia por motivos económicos y no por una
  restricción impuesta a priori sobre la consigna del convertidor;
  \item produce una solución de la que pueden recuperarse tensiones
  complejas con sentido físico, lo que la habilita como punto inicial
  para los solvers iterativos no convexos del flujo de cargas o del
  \gls{OPF} no lineal.
\end{enumerate}
